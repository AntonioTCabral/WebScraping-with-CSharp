...
namespace WebScraping.Core.Models
{
    public class Scrap
    {
        public static void getUrlContent(string baseUrl, string path, ConcurrentBag<ItemDescription> itemsDescription)
        {
            ...
            //Select all div with class js-navigation-item
            var childNodes = htmlDocument.DocumentNode.SelectNodes("//div[contains(@class,'js-navigation-item')]");

            //Loop for verification if div is file or folder
            Parallel.ForEach(childNodes, node =>
            {
                if (node.InnerText.Replace("\n", "").Trim() != ". .")
                {
                    if (node.SelectSingleNode("div[1]/svg").Attributes["aria-label"].Value == "Directory") // if is folder check the next div row to get  url folder
                    {
                        getUrlContent(baseUrl, node.SelectSingleNode("div[2]/span/a").Attributes["href"].Value, itemsDescription); //Recursion to check folder files 
                    }
                    else
                    {
                        itemsDescription.Add(getFileData(baseUrl, node)); //add files into List 
                    }
                }
            });
        }
